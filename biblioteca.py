...
class Livro:#CONSTRUTOR DA CLASSE
    def __init__(self, titulo, autor, ano_publicacao, genero): #INICIALIZANDO OS ATRIBUTOS
    ...
    def __str__(self): #RETORNANDO UMA STRING COM AS INFORMAÇÕES DO LIVRO
        return f"Gênero do livro: {self.genero}, Título: {self.titulo}, Autor: {self.autor} ({self.ano_publicacao})"
...
def adicionar_livro(titulo, autor, ano_publicacao, genero):
    novo_livro = Livro(titulo, autor, ano_publicacao, genero)
...
def listar_livros():
    if not biblioteca:
        ...
        print("\n") #LINHA EM BRANCO APOS LISTAR OS LIVROS

#FUNÇÃO PARA EXIBIR GRÁFICO DE LIVROS POR GENERO
def mostrar_grafico():
     #GERANDO UM GRAFICO DE LIVROS POR GENERO
    generos_unicos = sorted(set(generos))  # GENERO UNICO E ORDENADO
    ...
    plt.title('Número de Livros por Gênero de Publicação') #TITULO DO GRAFICO
...
def incluir_novos_livros():
    ...
    while True:
        ...
        adicionar_livro(titulo, autor, ano_publicacao, genero)

#ADICONANDO LIVROS PARA JÁ COMECAR COM ALGUS LIVROS
#CRIANDO UMA LISTA COM LIVROS INICIAIS
livros_iniciais = [ #TITULO, AUTOR, ANO
    ("Codido da Vinci", "Dan Brown", 2003 ,"Ficção"),
    ("O simbolo perdido", "Dan Brown", 2009 ,"Ficção"),
    ("Verity", "Colleen Hoover", 2018, "Romance"),
    ("A biblioteca da meia-noite", "Matt Haig", 2020, "Ficção"),
    ("A hipotese do amor", "Ali Hazelwood", 2021, "Romance"),
    ("It - A Coisa", "Stephen King", 1986, "Terror"),
    ("O iluminado", "Stephen King", 1977, "Terror"),
    ("Layla", "Colleen Hoover", 2020, "Romance"),
    ("Napoleão", "João C. Saldanha", 2017, "Biografia"),
    ("Martin lutero", "João C. Saldanha", 2017, "Biografia"),
    ("O pequeno príncipe", "Antoine de Saint-Exupéry", 1943, "Ficção"),
    ("Dom Quixote", "Miguel de Cervantes", 1605, "Ficção"),
    ("O senhor dos anéis", "J.R.R. Tolkien", 1954, "Ficção"),
    ("Python para desenvolvedores", "Wesley J. Chun", 2009, "Tecnologia"),
    ("Aprendendo Python", "Mark Lutz", 2013 ,"Tecnologia"),
    ("Automatize tarefas maçantes com Python", "Al Sweigart", 2015, "Tecnologia"),
    ("Fluent Python", "Luciano Ramalho", 2015, "Tecnologia"),
    ("PHP e MySQL", "Luke Welling e Laura Thomson", 2005, "Tecnologia"),
    ("JavaScript: The Good Parts", "Douglas Crockford", 2008, "Tecnologia"),
    ("Aprenda JavaScript", "Eloy Morais", 2017, "Tecnologia"),
    ("Domine o css", "Eloy Morais", 2017, "Tecnologia"),
    ("HTML5 e CSS3", "Eloy Morais", 2017, "Tecnologia"),
    ("PHP para iniciantes", "Eloy Morais", 2017, "Tecnologia"),
]
#ADICIONANDO OS LIVROS INICIAIS USANDO A FUNÇÃO
for titulo, autor, ano, genero in livros_iniciais:
    adicionar_livro(titulo, autor, ano, genero)
#FIM DAS ADICOES INICIAIS

#FUNCÃO PARA BUSCAR LIVROS PELO TITULO
def buscar_livro(titulo):
    resultados = [livro for livro in biblioteca if titulo.lower() in livro.titulo.lower()] #BUSCA INSENSIVEL A MAIUSCULAS E MINUSCULAS
...
def inicio():
    while True:
        ...
        print("\n") #LINHA EM BRANCO APOS PEGAR A OPÇÃO
